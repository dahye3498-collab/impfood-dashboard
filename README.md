...
- **이번 달 브리핑** — 최신 월 요약문 자동 생성 + 국가별 신호 감지(연속 증감·전년 대비 급변·신규 원산지). 신호 클릭 시 해당 국가로 필터.
- **CSV 내려받기 · 표 복사** — 월별 상세/국가별 데이터를 엑셀 호환 형식으로. 파일명에 필터 조건·기준월 자동 포함.
- **링크 공유 · 즐겨찾기** — 필터 상태가 URL에 담겨 링크로 공유 가능. 마지막 화면 자동 복원, 자주 쓰는 조합은 칩으로 저장.
- **추이 차트 3모드** — 연도 비교(비교 연도 직접 선택, 최대 4개) / 국가 비교(최근 24개월 상위 4개국) / 장기 추세(12개월 이동합, 계절성 제거)
- **계절성 히트맵** — 연도×월 매트릭스 + 평년 대비 편차 시그널(±15%)
- **구성 비중 추이** — 국가/부위/구분(냉동·냉장) 차원 토글, 최근 6개년 100% 누적 바 + 집중도(HHI). 국가별 바 클릭 시 해당 국가로 드릴다운.
- **FTA 소진 페이스 예측** — fta.json의 사용률·기준일(as_of)로 선형 추정한 소진 예상 시점 표시.
- **월 상세 드릴다운** — 월별 표·히트맵의 셀 클릭 시 그 달의 국가별·부위별 분해 + 전년 동월 증감 모달. 표의 연도 헤더 클릭으로 비교 연도 제외.
- **인쇄 리포트 모드** — 버튼 하나로 A4 월간 보고서(머리말 자동 생성)
- 필터(품명/구분/부위/국가) · 연도별 합계 · 월별 표(합계 행) · FTA · AI 질문(설정 시)
